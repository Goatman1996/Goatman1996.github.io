...
还有一个非常重要的细节：PBR要求在线性空间中计算，这个前置条件不满足的话，实现了也白瞎。关于线性空间，gamma空间，有时间再另起一篇吧。
...
对了，$a = \text{Roughness}^2$，这一步叫(Disney’s
reparameterization of a)，虚幻说迪斯尼说的，这么做好看
...
小结：  
当前唯一直接光源的渲染结果，且没加ambient，很黑很丑(我做到这的时候就是这么觉得的)，但是跟着做到这的读者不用担心。  
...
与确定方向直接光不同，间接光的数据来源来源，IBL的基本思路是，对于某个方向的光，即是对环境贴图在该方向上进行采样的结果。
...
我想很多游戏前端都会如此吧，想着入渲染或是入图形学的门，尝试了一次又一次，在各种阶段各种地方或倒下或止步。搁置久了又忘了。每每捡起来，又是一次三分钟热度的`由头来过`。起码，我便是这么个俗人。
